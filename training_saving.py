...
def plot_results(all_episode_return_data, all_evaluation_results, seeds):
    ...
    ax.set_title(f'Training and Evaluation Curve', fontsize=16)
    ax.legend(loc='upper left', fontsize=12)
    ax.grid(True, alpha=0.3)
    
    plt.tight_layout()
    plt.savefig('learning_curveX.pdf', dpi=300)
    print("Plot saved as 'learning_curve3.pdf'")
    plt.show()
